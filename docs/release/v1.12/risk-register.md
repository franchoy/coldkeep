...
| CK-112-R004 | CLI behavior drift during routing | S1 | CLI/engine | all migration phases | open (partial) | Phase 5: snapshot list/show/stats/diff parity proven by `internal/engine/snapshot_engine_test.go` and CLI routing tests. Remaining commands (gc, restore, store, remove) open. |
| CK-112-R005 | JSON output drift during routing | S1 | CLI/rendering | all migration phases | open (partial) | Phase 5: JSON envelope and field shape for snapshot commands verified. Remaining commands (gc, restore, store, remove) open. |
