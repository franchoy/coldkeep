...
func TestPhase9PackedFileDeepVerificationBackendProof(t *testing.T) {
	...
	t.Run("packed-companion-mixed-and-corrupt-packed-subset", provePhase9PackedMixedRecipe)
	t.Run("missing-placement-fails-closed", provePhase9MissingPlacementFailsClosed)
	t.Run("malformed-packed-range-fails-closed", provePhase9MalformedPackedRangeFailsClosed)
}

func provePhase9PackedMixedRecipe(t *testing.T) {
	dbconn, _, fileID := testutils.SetupStoredFileForVerification(t, "phase9-packed-mixed.bin", 4*1024*1024)
	defer dbconn.Close()
	requirePhase9PackedCompanionCounts(t, dbconn, fileID)
	...
	firstChunkID, secondChunkID := phase9FirstTwoPackedChunkIDs(t, dbconn, fileID)
...
func requirePhase9PackedCompanionCounts(t *testing.T, dbconn *sql.DB, fileID int64) {
	t.Helper()
...
func phase9FirstTwoPackedChunkIDs(t *testing.T, dbconn *sql.DB, fileID int64) (int64, int64) {
	t.Helper()
	...
	return firstChunkID, secondChunkID
}

func provePhase9MissingPlacementFailsClosed(t *testing.T) {
...
func provePhase9MalformedPackedRangeFailsClosed(t *testing.T) {
...
func TestPhase9SimulateGCPostgresCoordinationIntegration(t *testing.T) {
	testgate.RequireDB(t)
	fixture := newPhase9SimulateGCFixture(t)
	busy := fixture.run(t)
	requirePhase9BusySimulation(t, busy)
	fixture.release(t)
	uncontended := fixture.run(t)
	requirePhase9ExactSimulation(t, uncontended)
}

type phase9SimulateGCFixture struct {
...
func newPhase9SimulateGCFixture(t *testing.T) *phase9SimulateGCFixture {
	t.Helper()
	tmp := t.TempDir()
	...
	fixture := &phase9SimulateGCFixture{
	...
	t.Cleanup(func() {
		if !fixture.released {
			_ = fixture.lease.Release()
		}
	})
	return fixture
}

func (f *phase9SimulateGCFixture) run(t *testing.T) testutils.CLIExecResult {
	t.Helper()
	return testutils.RunColdkeepCommand(t, f.repoRoot, f.binPath, f.env, "simulate", "gc", "--output", "json")
}

func (f *phase9SimulateGCFixture) release(t *testing.T) {
	t.Helper()
...
func requirePhase9BusySimulation(t *testing.T, busy testutils.CLIExecResult) {
	t.Helper()
...
func requirePhase9ExactSimulation(t *testing.T, uncontended testutils.CLIExecResult) {
	t.Helper()
