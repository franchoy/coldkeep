...
| G13 | Repair command supports deterministic batch orchestration with stable per-item JSON outcomes | `repair ref-counts --batch` processes targets in deterministic order, reports per-item `status/error`, preserves fail-fast semantics, and surfaces invariant metadata on execution failures | `TestRunRepairCommandBatchJSONPartialFailure`, `TestRunRepairCommandBatchInvariantFailureUsesVerifyExitAndMetadata` | covered |
