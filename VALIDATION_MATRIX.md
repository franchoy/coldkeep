...
| Atomic restore replacement on the single-node local filesystem model | Restore path writes temp + fsync + atomic rename | `TestRestoreAtomicityWithTestHook`, `TestRestoreAtomicityWithCorruption` (explicit atomicity and cleanup), `TestStoreGCRestore`, `TestSampleDatasetEndToEnd` | covered |
...
| Atomic restore failure-mode tests | Simulate restore failures before/after rename; verify original output file is preserved, no partial/corrupt final file is visible, and temp files are cleaned up | `TestRestoreAtomicityWithTestHook`, `TestRestoreAtomicityWithCorruption` | completed |
