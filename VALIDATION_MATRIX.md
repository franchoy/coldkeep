...
| G14 | Snapshot-retained content is GC-safe | Current recipe liveness is contributed once by completed logical files having at least one `physical_file` mapping, while independent `snapshot_file` roots preserve snapshot-only recipes and placements; delete-by-ID remains blocked but stored-path unlink is allowed | `TestV11316SnapshotOnlyRetentionCanBeCreatedThroughNormalRemove`, current-root transition/storage tests, `TestRunGCDoesNotDeleteSnapshotRetainedContainer`, `TestAdversarialG14SnapshotRetainedGCGuardUnderChurn` | covered; SQLite and PostgreSQL plain/AES-GCM, legacy/packed GC, byte-identical restore, unreachable/current-live controls, and pin interleaving certified |
| G15 | Snapshot deletion only changes metadata and future GC eligibility | Deleting a snapshot removes only snapshot metadata; one of multiple roots retains content, while deleting the last root makes a zero-current unpinned recipe eligible for a later GC pass. An active restore pin postpones cleanup | `TestV11316SnapshotOnlyRetentionCanBeCreatedThroughNormalRemove`, `TestDeleteSnapshotRemovesSnapshotRowsOnly`, `TestAdversarialG17RetentionRootTransitionChurn` | covered; dry-run/live parity and pin-release reclamation certified |
