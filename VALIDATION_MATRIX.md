...
| Batch repair orchestration semantics (Phase 8) | Add deterministic batch maintenance contract for `repair ref-counts --batch` (continue-on-error/fail-fast modes, per-item reporting, stable JSON/text semantics) with integration coverage | Phase 8 track | deferred |
