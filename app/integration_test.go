...
// They are skipped unless CAPSULE_TEST_DB=1 is set.
//
// Run (example):
//   CAPSULE_TEST_DB=1 DB_HOST=localhost DB_PORT=5432 DB_USER=capsule DB_PASSWORD=capsule DB_NAME=capsule go test ./app -v

func requireDB(t *testing.T) {
	t.Helper()
	if os.Getenv("CAPSULE_TEST_DB") == "" {
		t.Skip("Set CAPSULE_TEST_DB=1 to run integration tests")
	}
}
...
func applySchema(t *testing.T, db *sql.DB) {
	...
	if p := os.Getenv("CAPSULE_SCHEMA_PATH"); p != "" {
		b, err := os.ReadFile(p)
		if err != nil {
	...
	t.Fatalf("could not find db/init.sql; set CAPSULE_SCHEMA_PATH to an absolute path")
}
...
func TestRoundTripStoreRestore(t *testing.T) {
	...
	_ = os.Setenv("CAPSULE_STORAGE_DIR", storageDir)
	resetStorage(t)
