...
func (p *darwinPending) preparePublish() error {
...
func (p *darwinPending) publishOverwrite() error {
...
func (p *darwinPending) publishNoReplace() error {
	...
	case darwinRenameNoReplaceUnsupported(err):
...
func (p *darwinPending) syncParent() error {
...
func (p *darwinPending) closeWriter() error {
	if p.writerFile == nil {
		return nil
	}
...
func (p *darwinPending) removeTemporary() error {
...
func (p *darwinPending) closeObject() error {
	if p.objectFD < 0 {
		return nil
	}
...
func (p *darwinPending) closeParent() error {
	if p.parentFD < 0 {
		return nil
	}
...
func darwinOpenParent(request Request) (int, string, string, error) {
	...
	for _, part := range parts[:len(parts)-1] {
		next, openErr := darwinOpenParentComponent(fd, part)
		if openErr != nil {
			_ = unix.Close(fd)
			return -1, "", "", openErr
		}
...
func darwinOpenParentComponent(parentFD int, part string) (int, error) {
	...
	return unix.Openat(parentFD, part, unix.O_RDONLY|unix.O_DIRECTORY|unix.O_CLOEXEC|unix.O_NOFOLLOW, 0)
}

func darwinRenameNoReplaceUnsupported(err error) bool {
	return errors.Is(err, unix.ENOTSUP) || errors.Is(err, unix.EOPNOTSUPP) || errors.Is(err, unix.EINVAL) || errors.Is(err, unix.ENOSYS)
}

func darwinLinkUnsupported(err error) bool {
	return errors.Is(err, unix.ENOTSUP) || errors.Is(err, unix.EOPNOTSUPP) || errors.Is(err, unix.ENOSYS) || errors.Is(err, unix.EPERM)
}
