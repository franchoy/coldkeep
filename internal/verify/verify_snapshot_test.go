...
func TestVerifySystemStandardPassesWithConsistentSnapshotReachability(t *testing.T) {
	...
	if _, err := dbconn.Exec(`INSERT INTO snapshot_file (snapshot_id, path, logical_file_id) VALUES ($1, $2, $3)`, "snap-ok-1", "docs/snapshot-ok.txt", logicalID); err != nil {
...
func TestVerifySystemStandardDetectsOrphanSnapshotLogicalReference(t *testing.T) {
	...
	if _, err := dbconn.Exec(`INSERT INTO snapshot_file (snapshot_id, path, logical_file_id) VALUES ($1, $2, $3)`, "snap-orphan-1", "docs/missing.txt", int64(9999)); err != nil {
...
func TestVerifySystemStandardDetectsSnapshotInvalidLifecycleState(t *testing.T) {
	...
	if _, err := dbconn.Exec(`INSERT INTO snapshot_file (snapshot_id, path, logical_file_id) VALUES ($1, $2, $3)`, "snap-lifecycle-1", "docs/snapshot-aborted.txt", logicalID); err != nil {
		t.Fatalf("insert snapshot_file row: %v", err)
	}
...
func TestVerifySystemStandardDetectsSnapshotRetainedMissingChunkGraph(t *testing.T) {
	...
	if _, err := dbconn.Exec(`INSERT INTO snapshot_file (snapshot_id, path, logical_file_id) VALUES ($1, $2, $3)`, "snap-graph-1", "docs/snapshot-missing-graph.txt", logicalID); err != nil {
		t.Fatalf("insert snapshot_file row: %v", err)
	}
