...
func (s *Service) simulateGC(ctx context.Context, opts SimulationOptions) (*SimulationResult, error) {
	...
	result := &SimulationResult{
		...
		Summary: map[string]any{
			...
			"fully_reclaimable_containers":             plan.Summary.FullyReclaimableContainers,
			"partially_dead_containers":                plan.Summary.PartiallyDeadContainers,
			"packed_blocks_live":                       plan.Summary.PackedBlocksLive,
			"packed_blocks_dead":                       plan.Summary.PackedBlocksDead,
			"packed_bytes_live":                        plan.Summary.PackedBytesLive,
			"packed_bytes_reclaimable":                 plan.Summary.PackedBytesReclaimable,
			"retained_dead_bytes_due_to_packed_blocks": plan.Summary.RetainedDeadBytesDueToPackedBlocks,
			"affected_containers":                      len(plan.AffectedContainers),
		},
		GC: &GCSimulationResult{
			...
			Summary: GCSimulationSummary{
				...
				PackedBlocksLive:                   plan.Summary.PackedBlocksLive,
				PackedBlocksDead:                   plan.Summary.PackedBlocksDead,
				PackedBytesLive:                    plan.Summary.PackedBytesLive,
				PackedBytesReclaimable:             plan.Summary.PackedBytesReclaimable,
