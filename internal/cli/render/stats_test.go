...
func TestRenderStatsHumanPrintsChunkerVersionsInSortedOrder(t *testing.T) {
	...
	input := &StatsResult{
		...
		BlockLayout: observability.BlockLayoutStats{
			StorageBlocksCount:        100,
			ChunkBlockRefsCount:       3200,
			AvgChunksPerBlock:         32,
			AvgBlockPlaintextSize:     2 * 1024 * 1024,
			AvgBlockStoredSize:        1900 * 1024,
			LogicalBytes:              200 * 1024 * 1024,
			CompressedBytes:           140 * 1024 * 1024,
			StoredBytes:               150 * 1024 * 1024,
			CompressionSizeRatio:      0.700,
			CompressionFactor:         1.428,
			PhysicalSizeRatio:         0.750,
			PhysicalFactor:            1.333,
			CompressedBlocks:          80,
			UncompressedBlocks:        20,
			AvgBlockFillRatio:         0.95,
			LegacyBlockCount:          12,
			PackedBlockCount:          100,
			CodecDistribution:         map[string]int64{"none": 100},
			CompressionCodecBreakdown: map[string]int64{"none": 20, "zstd": 80},
		},
	...
	for _, want := range []string{
		...
		"logical_bytes: 200.0 MiB",
		"compressed_bytes: 140.0 MiB",
		"stored_bytes: 150.0 MiB",
		"compression_size_ratio: 0.700",
		"compression_factor: 1.428",
		"physical_size_ratio: 0.750",
		"physical_factor: 1.333",
		"compressed_blocks: 80",
		"uncompressed_blocks: 20",
		"avg_block_fill_ratio: 0.950",
		"legacy_block_count: 12",
		"packed_block_count: 100",
		"codec[none]: 100",
		"compression_codec[none]: 20",
		"compression_codec[zstd]: 80",
