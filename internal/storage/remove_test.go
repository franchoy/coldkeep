...
func TestRemoveFailsOnInvalidLiveRefCountTransition(t *testing.T) {
	...
	if _, err := dbconn.Exec(
		`INSERT INTO physical_file (path, logical_file_id, is_metadata_complete) VALUES ($1, $2, 1)`,
		filepath.Join(t.TempDir(), "zero-ref-file.bin"), fileID,
	); err != nil {
...
func assertStoredPathRemovePreservesPayloadState(t *testing.T, beforeState, afterState removestate.Snapshot) {
	...
	if !reflect.DeepEqual(beforeState.FileChunkIDs, afterState.FileChunkIDs) || !reflect.DeepEqual(beforeState.ChunkPinCounts, afterState.ChunkPinCounts) {
		t.Fatalf("stored-path unlink must preserve logical chunk ownership: before=%+v after=%+v", beforeState, afterState)
	}
	for chunkID, beforeLive := range beforeState.ChunkLiveRefs {
		if got := afterState.ChunkLiveRefs[chunkID]; got != beforeLive-1 {
			t.Fatalf("last stored-path unlink must deactivate recipe chunk %d exactly once: before=%d after=%d", chunkID, beforeLive, got)
		}
	}
}
