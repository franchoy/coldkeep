...
// Compression Config Integration Tests

// TestCompressionConfigDoeNotAffectWriteBehavior verifies that changing
// the repository compression config does NOT affect write behavior. This is a
// critical safety check: Phase 1.4 adds config but doesn't enable
// compression behavior. The write path remains uncompressed regardless of config.
func TestCompressionConfigDoesNotAffectWriteBehavior(t *testing.T) {
	...
	// Verify initial compression config is "none"
	tx, err := dbconn.Begin()
	if err != nil {
		t.Fatalf("begin tx to verify initial config: %v", err)
	}
	defaultCodec, err := GetDefaultCompression(tx)
	_ = tx.Rollback()
	if err != nil {
		t.Fatalf("GetDefaultCompression: %v", err)
	}
	if defaultCodec != "none" {
		t.Fatalf("expected initial compression='none', got %q", defaultCodec)
	}

	// Verify initial compression level is 3
	tx, err = dbconn.Begin()
	if err != nil {
		t.Fatalf("begin tx to verify initial level: %v", err)
	}
	defaultLevel, err := GetDefaultCompressionLevel(tx)
	_ = tx.Rollback()
	if err != nil {
		t.Fatalf("GetDefaultCompressionLevel: %v", err)
	}
	if defaultLevel != 3 {
		t.Fatalf("expected initial compression_level=3, got %d", defaultLevel)
	}

	// Change compression config to "none" (explicitly set to itself)
	// This simulates a user acknowledging the config option even though
	// they're not changing behavior yet
	tx, err = dbconn.Begin()
	if err != nil {
		t.Fatalf("begin tx to set config: %v", err)
	}
	...
	if err := tx.Commit(); err != nil {
		t.Fatalf("commit config set: %v", err)
	}

	// Verify config persists
	tx, err = dbconn.Begin()
	if err != nil {
		t.Fatalf("begin tx to verify config: %v", err)
	}
	codec, err := GetDefaultCompression(tx)
	_ = tx.Rollback()
	if err != nil {
		t.Fatalf("GetDefaultCompression after set: %v", err)
	}
	if codec != "none" {
		t.Fatalf("config didn't persist: expected none, got %q", codec)
	}

	// Change compression level to 7
	tx, err = dbconn.Begin()
	if err != nil {
		t.Fatalf("begin tx to set level: %v", err)
	}
	if err := SetDefaultCompressionLevel(tx, 7); err != nil {
		_ = tx.Rollback()
		t.Fatalf("SetDefaultCompressionLevel(7): %v", err)
	}
	if err := tx.Commit(); err != nil {
		t.Fatalf("commit level set: %v", err)
	}

	// Verify level persists
	tx, err = dbconn.Begin()
	if err != nil {
		t.Fatalf("begin tx to verify level: %v", err)
	}
	level, err := GetDefaultCompressionLevel(tx)
	_ = tx.Rollback()
	if err != nil {
		t.Fatalf("GetDefaultCompressionLevel after set: %v", err)
	}
	if level != 7 {
		t.Fatalf("level didn't persist: expected 7, got %d", level)
	}

	// CRITICAL INVARIANT: Config changes do NOT affect the write pipeline.
	// Blocks are still written with codec="plain" (v1.8 compatible, uncompressed).
