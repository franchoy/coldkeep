...
func claimLogicalFile(db *sql.DB, file_info os.FileInfo, fileHash string) (fileID int64, err error) {
	...
	if insErr == sql.ErrNoRows {
		...
		case "COMPLETED":
			...
			tx.Rollback() // Don't hold locks while waiting
		...
		case "PROCESSING":
			...
			for keep_waiting := true; keep_waiting; {
				...
				case "ABORTED":
					...
					status = finalStatus // Update status to break the loop and retry storing
					keep_waiting = false
				}
			}
		}
...
func claimChunk(db *sql.DB, chunkHash string, chunksize int64) (chunkID int64, chunkstatus string, err error) {
	...
	if insErr == nil {
		...
		if err := tx.QueryRow(`SELECT id, status FROM chunk WHERE chunk_hash = $1 AND size = $2`, chunkHash, chunksize).Scan(&chunkID, &status); err != nil {
			return 0, "", err
		}
		switch status {
		case "COMPLETED":
...
func StoreFileWithDB(db *sql.DB, path string) (err error) {
	...
	fileID, err := claimLogicalFile(db, file_info, fileHash)
	if err != nil {
		return err
	}
	...
	for _, chunkData := range chunks {
		...
		if err2 := tx.QueryRow(
			`UPDATE chunk SET container_id = $1, chunk_offset = $2, status = 'COMPLETED' WHERE id = $3 RETURNING id`,
			containerID,
			offset,
			claimedChunkID,
		).Scan(&claimedChunkID); err2 != nil {
			err = err2
			return err
		}
